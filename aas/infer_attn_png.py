...
def eval_model(args):
    ...
    for anno_index in tqdm(anno_indices):
        ...
        save_attn = torch.cat([x[:, :, -caption_token_length:, image_token_start_index:image_token_end_index] for x in save_attn])
        save_attn = save_attn.mean(dim=(0, 1)).reshape(caption_token_length, args.feature_height, args.feature_width)
